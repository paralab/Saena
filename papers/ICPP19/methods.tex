...
Since in this case $B$ is symmetric, we can work with its local transpose. Instead of working with $B_i$'s, we consider their local transpose $BT_i$ in Figure~\ref{fig:partition4}.
...
If we partition $B$ in column blocks, then we don't even need $B$ to be symmetric, but since we want to consider the same partitioning for all the matrices in our algorithm and we don't want to change the partition for one matrix only to perform \recmm, we assumed $B$ being symmetric.
...
There are at least two advantages for changing Algorithm~\ref{alg:comm1} to Algorithm~\ref{alg:comm2}. First, we can sort and add duplicates on a subset of entries of $C_i$ (compare line 9 of Algorithm~\ref{alg:comm1} with line $8$ of Algorithm~\ref{alg:comm2}. The other benefit is having better divided blocks of B after splitting.
