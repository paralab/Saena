...
\label{sec:case2}
When $A$ is horizontal, i.e. its row size is less than or equal to its column size, we halve $A$ by column based on its column size (Figure ~\ref{fig:case2}). Since the row size of $B$ equals the column size of $A$, we halve $B$ by row, so it will be a split similar to $A$, but horizontally. 

\begin{figure}[tbh]
 \centering
 \Description{Description}
 \includegraphics[width=6cm,height=2.7cm]{./figures/case2_001.pdf}
 \caption{Case 2: When A is horizontal, split A by column and B by row.}
 \label{fig:case2}
\end{figure}
...
\begin{figure}[tbh]
 ...
 \caption{Case 3: When A is vertical, split A by row and B by column.}
 \label{fig:case3}
\end{figure}
...
\mr{highlight that the matmult is the dominant part, and the part that is complex in parallel.}
...
\begin{figure}[tbh]
 ...
 \caption{Partition the matrices between the processors in row blocks.}
 \label{fig:partition}
\end{figure}
...
\begin{figure}[tbh]
 ...
 \caption{This figure shows how the matrices are split into sub-blocks in Part 1.}
 \label{fig:part1b}
\end{figure}
...
\mr{explain better}Using this method, we need to add duplicates twice (explain duplicates). Once, after doing the multiplication; Second time, after sorting the result globally (using HykSort \cite{Sundar:2013}) and putting the entries on the correct processors (Algorithm~\ref{alg:part2}).
