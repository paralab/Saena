...
\label{sec:case2}
When A is horizontal, i.e. its row size is less than or equal to its column size, we halve A by column based on its cloumn size (add Figure\ref{fig:case2}). Since row size of B eqauls column size of A, we halve B by row, so it will a smiliar split, but horizontally.
...
\label{sec:case3}
When A is vertical, i.e. its column size is greater than its column size, we halve A by row based on its cloumn size and halve B by column (add Figure\ref{fig:case3}). In contrary to the previous case, column size of B is not related to row size of A, so they are splitted independently.
