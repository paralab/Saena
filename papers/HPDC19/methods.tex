...
\mr{too abrupt. add the following...edit as neccessary}

In this section, we present our modifications to the triple matrix multiplications that are part of the AMG setup process. First give a brief overview of the AMG setup process, followed by our specific changes.

\subsection{AMG Setup}
...
\label{sec:case1}
At the start of the recursive function, the number of nonzero rows of A ($A\_nnz\_row$) and nonzero columns of B ($B\_nnz\_col$) are being computed. A threshold for $\nnzsz = A\_nnz\_row \times B\_nnz\_col$ is being set (for our experiments we use $10M$ and sometimes $20M$).
We continue splitting the matrices until the threshold is reached. Then, we preform the multiplication. We have implemented two methods for this case: dense struture and hashmap.

%First method uses a dense matrix.
%The matrices are ordered as column-major.
In the first method, a dense matrix of size $\nnzsz = A\_nnz\_row \times B\_nnz\_col$ is initialized to $0$. Each nonzero of $B$ is multiplied by its corresponding nonzero of $A$ and the result will be added to the corresponding index in the dense matrix. At the end, we go through the dense matrix and add the nonzeros to the final multiplication matrix.

When $\nnzsz$ gets larger, it becomes inefficient to traverse the whole dense matrix and check for nonzeros in the final stage. To solve this issue, we have implemented another method that uses hashmaps. The entry's index is the key and its value is the hashmap's value. When we want to add the multipliation of nonzeros of $A$ and $B$ to the hashmap, we check if the index exists in the map. If it exists the value is being added. Otherwise a new entry will be added to the hashmap.

In Figure~\ref{fig:lap60}, we compare the two methods for a 3D Poisson problem of size $216k$. For $0 \leq \nnzsz \leq 10M$, in $1M$ steps, we count how many times each method is faster than the other one. For the lower range the first method is better and for the higher range the second one.
...
A combination of these two methods would give us the best of both. The dense method is being used for the lower range and the hashmap for the higher range. Figure~\ref{fig:mix} shows the comparison of the mixed method with when each method is being used separately.
