...
//        if(rank==1) cout << vElement_remote[i] << endl;
    ...
    vSend_t     = (unsigned long*)malloc(sizeof(unsigned long) * 2*vIndexSize_t); // todo: check datatype here.
    vecValues_t = (unsigned long*)malloc(sizeof(unsigned long) * 2*recvSize_t);
