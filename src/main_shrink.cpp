...
//    std::vector<bool> sh_lev_vec = {false, false, false, false, false, false, false}; // set at which levels shrinking should happen.
...
//    solver.switch_repartition(false);
    ...
    solver.switch_repartition(true);
    std::vector<bool> sh_lev_vec = {false, true, false, false, false, false, false, false, false}; // set at which levels shrinking should happen.
    solver.set_shrink_levels(sh_lev_vec);
    std::vector<int> sh_val_vec = {0, 2, 0, 0, 0, 0, 0, 0, 0}; // set at which levels shrinking should happen.
    solver.set_shrink_values(sh_val_vec);
//    float repartition_threshold = std::stof(argv[2]);
//    solver.set_repartition_threshold(repartition_threshold);
...
//    double average1, average2, average3;
    ...
    for (int i = 0; i < matvec_iter_warm; i++) {
