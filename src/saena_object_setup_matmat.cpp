...
double t_init_prep = 0, t_mat = 0, t_comp_GR = 0, t_comp_zfp = 0, t_decomp_GR = 0, t_decomp_zfp = 0, t_prep_iter = 0,
       t_fast_mm = 0, t_sort = 0, t_wait = 0;
...
//            printf("mkl_dcsrmultcsr result: %d\n", info);
            ...
            MKL_INT i = 0, j = 0, ii = 0;
            const int B_C_SZ   = B.col_sz;
            const int ATHRSHLD = A.row_offset - 1;
            const int BTHRSHLD = B.col_offset;

            // Cmkl_c_scan is 1-indexed, so i starts from 1. That's why we use ii to start from 0.
            for (j = 0; j < B_C_SZ; ++j) {
...
//        std::cout << "\nCase2:\nA1: nnz = " << A1.nnz << std::endl;
            if (A1.nnz != 0) {
                for (nnz_t i = 0; i < A1.col_sz; ++i) {
                    for (nnz_t j = A1.col_scan[i] - 1; j < A1.col_scan[i + 1] - 1; ++j) {
...
//        std::cout << "\nA2: nnz = " << A2.nnz << std::endl;
            if (A2.nnz != 0) {
                for (nnz_t i = 0; i < A2.col_sz; ++i) {
                    for (nnz_t j = A2.col_scan[i] - 1; j < A2.col_scan[i + 1] - 1; ++j) {
...
//        std::cout << "\nCase2:\nB1: nnz = " << B1.nnz << std::endl;
            if (B1.nnz != 0) {
                for (nnz_t i = 0; i < B1.col_sz; ++i) {
                    for (nnz_t j = B1.col_scan[i] - 1; j < B1.col_scan[i + 1] - 1; ++j) {
//                        if(B1.r[j] >= B1.row_sz)
...
//        std::cout << "\nB2: nnz = " << B2.nnz << std::endl;
            if (B2.nnz != 0) {
                for (nnz_t i = 0; i < B2.col_sz; ++i) {
                    for (nnz_t j = B2.col_scan[i] - 1; j < B2.col_scan[i + 1] - 1; ++j) {
...
//                       A.nnz, A1.nnz, A2.nnz, B.nnz, B1.nnz, B2.nnz, A.row_sz, A.col_sz, A_col_size_half, A.col_sz, B.col_sz);
...
//                   A.nnz, A1.nnz, A2.nnz, B.nnz, B1.nnz, B2.nnz, A.row_sz, A.col_sz, A.col_sz, B.col_sz, B_col_size_half);
...
int saena_object::matmat(saena_matrix *A, saena_matrix *B, saena_matrix *C, const bool assemble/* = true*/, const bool print_timing/* = false*/, const bool B_trans/* = true*/){
    ...
    int matmat_iter_warmup = 10, matmat_iter = 20;
    ...
    for(nnz_t i = 0; i < A->nnz_l; i++){
        ...
        ASSERT( fabs(A->entry[i].val) > ALMOST_ZERO, A->entry[i] );
    }
#endif
...
//    assert(Bcsc.nnz == (Bcsc.col_scan[Bcsc.col_sz] - Bcsc.col_scan[0]));
    ...
    if(!print_timing){
        ...
        t_init_prep = 0, t_mat = 0, t_comp_GR = 0, t_comp_zfp = 0, t_decomp_GR = 0, t_decomp_zfp = 0, t_prep_iter = 0,
        t_fast_mm = 0, t_sort = 0, t_wait = 0;
        ...
        if(nprocs > 1 && !rank) {
            if(zfp_perform) {
                std::cout << "zfp: orig sz (rank0) = " << zfp_orig_sz << ", comp sz (rank0) = " << zfp_comp_sz
                          << ", saving %" << std::setprecision(2) << 100 * (1 - zfp_rate) << " (average)\n";
            }else{
                std::cout << "zfp: not performed! rate: " << zfp_rate << std::endl;
            }
