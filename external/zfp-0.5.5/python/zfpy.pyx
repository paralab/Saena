...
cdef zfp_field* _init_field(np.ndarray arr):
    ...
    cdef zfp_field* send_field
    cdef void* pointer = arr.data

    strides = [int(x) / arr.itemsize for x in arr.strides[:ndim]]

    if ndim == 1:
        send_field = zfp_field_1d(pointer, ztype, shape[0])
        zfp_field_set_stride_1d(send_field, strides[0])
    elif ndim == 2:
        send_field = zfp_field_2d(pointer, ztype, shape[1], shape[0])
        zfp_field_set_stride_2d(send_field, strides[1], strides[0])
    elif ndim == 3:
        send_field = zfp_field_3d(pointer, ztype, shape[2], shape[1], shape[0])
        zfp_field_set_stride_3d(send_field, strides[2], strides[1], strides[0])
    elif ndim == 4:
        send_field = zfp_field_4d(pointer, ztype, shape[3], shape[2], shape[1], shape[0])
        zfp_field_set_stride_4d(send_field, strides[3], strides[2], strides[1], strides[0])
    else:
...
cpdef bytes compress_numpy(
    ...
    # Setup send_zfp structs to begin compression
    cdef zfp_field* send_field = _init_field(arr)
    cdef zfp_stream* send_stream = zfp_stream_open(NULL)

    cdef zfp_type ztype = zfp_type_none;
    cdef int ndim = arr.ndim;
    _set_compression_mode(send_stream, ztype, ndim, tolerance, rate, precision)

    # Allocate space based on the maximum size potentially required by send_zfp to
    # store the compressed array
    cdef bytes compress_str = None
    cdef size_t maxsize = zfp_stream_maximum_size(send_stream, send_field)
    try:
        with Memory(maxsize) as data:
            bstream = stream_open(data, maxsize)
            zfp_stream_set_bit_stream(send_stream, bstream)
            zfp_stream_rewind(send_stream)
            # write the full header so we can reconstruct the numpy array on
            # decompression
            if write_header and zfp_write_header(send_stream, send_field, HEADER_FULL) == 0:
                raise RuntimeError("Failed to write header to send_stream")
            with nogil:
                compressed_size = zfp_compress(send_stream, send_field)
            if compressed_size == 0:
                raise RuntimeError("Failed to write to send_stream")
        ...
        zfp_stream_close(send_stream)
        stream_close(bstream)

    return compress_str

cdef view.array _decompress_with_view(
    zfp_field* send_field,
    zfp_stream* send_stream,
):
    cdef zfp_type ztype = send_field[0]._type
    dtype = ztype_to_dtype(ztype)
    format_type = dtype_to_format(dtype)

    shape = (send_field[0].nw, send_field[0].nz, send_field[0].ny, send_field[0].nx)
    ...
    with nogil:
        zfp_field_set_pointer(send_field, pointer)
        ret = zfp_decompress(send_stream, send_field)
    if ret == 0:
        raise RuntimeError("error during send_zfp decompression")
    return decomp_arr
...
cdef _decompress_with_user_array(
    ...
    with nogil:
        zfp_field_set_pointer(send_field, out)
        ret = zfp_decompress(send_stream, send_field)
    if ret == 0:
        raise RuntimeError("error during send_zfp decompression")
...
cdef _set_compression_mode(
    ...
    if tolerance >= 0:
        zfp_stream_set_accuracy(send_stream, tolerance)
    elif rate >= 0:
        zfp_stream_set_rate(send_stream, rate, ztype, ndim, 0)
    elif precision >= 0:
        zfp_stream_set_precision(send_stream, precision)
    else:
        zfp_stream_set_reversible(send_stream)
...
cpdef np.ndarray _decompress(
    ...
    cdef zfp_field* send_field = zfp_field_alloc()
    cdef bitstream* bstream = stream_open(
        comp_data_pointer,
        len(compressed_data)
    )
    cdef zfp_stream* send_stream = zfp_stream_open(bstream)
    ...
    try:
        ...
        send_field[0].nx, send_field[0].ny, send_field[0].nz, send_field[0].nw = zshape
        zfp_field_set_type(send_field, ztype)
        ...
        # send_field[0].sx, send_field[0].sy, send_field[0].sz, send_field[0].sw = strides

        if out is None:
            output = np.asarray(_decompress_with_view(send_field, send_stream))
        else:
            ...
            _decompress_with_user_array(send_field, send_stream, <void *>output.data)
...
cpdef np.ndarray decompress_numpy(
    ...
    try:
        if zfp_read_header(send_stream, send_field, HEADER_FULL) == 0:
            raise ValueError("Failed to read required send_zfp header")
        output = np.asarray(_decompress_with_view(send_field, send_stream))
    finally:
        zfp_field_free(send_field)
        zfp_stream_close(send_stream)
