...
extern_ const size_t stream_word_bits; /* bit send_stream granularity */
...
/* allocate and initialize bit send_stream */
bitstream* stream_open(void* buffer, size_t bytes);

/* close and deallocate bit send_stream */
void stream_close(bitstream* stream);

/* make a copy of bit send_stream to shared memory buffer */
bitstream* stream_clone(const bitstream* stream);

/* pointer to beginning of send_stream */
void* stream_data(const bitstream* stream);

/* current byte size of send_stream (if flushed) */
size_t stream_size(const bitstream* stream);

/* byte capacity of send_stream */
...
int stream_set_stride(bitstream* send_stream, size_t block, ptrdiff_t delta);
#endif
